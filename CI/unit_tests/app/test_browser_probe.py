...
from software.app.browser_probe import (
    BrowserProbeRequest,
    BrowserProbeResult,
    _parse_probe_stdout,
    probe_browser_environment,
    run_browser_probe_cli_from_argv,
    run_browser_probe_subprocess,
)
...
class BrowserProbeTests(unittest.TestCase):
    def test_request_token_roundtrip_filters_empty_browser_names(self) -> None:
        ...
        self.assertFalse(restored.headless)
        self.assertEqual(restored.browser_preference, ["edge", "chrome"])
    ...
    def test_probe_browser_environment_reports_success_and_quits_driver(self) -> None:
        ...
        with patch("software.app.browser_probe.create_playwright_driver", return_value=(fake_driver, "edge")), \
             patch("software.app.browser_probe.time.monotonic", side_effect=[0.0, 0.2]):
            result = probe_browser_environment(BrowserProbeRequest(headless=True, browser_preference=["edge"]))

        self.assertTrue(result.ok)
        self.assertEqual(result.browser, "edge")
        self.assertEqual(result.message, "浏览器环境快速检查通过")
        self.assertTrue(fake_driver.quit_called)

    def test_probe_browser_environment_reports_launch_failure(self) -> None:
        with patch("software.app.browser_probe.create_playwright_driver", side_effect=RuntimeError("boom")), \
             patch("software.app.browser_probe.time.monotonic", side_effect=[0.0, 0.5]):
            result = probe_browser_environment(BrowserProbeRequest(headless=True, browser_preference=["edge"]))
    ...
    def test_run_browser_probe_subprocess_tolerates_invalid_utf8_bytes(self) -> None:
        fake_process = _CompletedProbeProcess(
    ...
    def test_run_browser_probe_subprocess_rejects_invalid_response(self) -> None:
        ...
        self.assertIn("stderr boom", result.message)

    def test_run_browser_probe_subprocess_times_out_and_kills_process(self) -> None:
        fake_process = _PendingProbeProcess()

        with patch("software.app.browser_probe.subprocess.Popen", return_value=fake_process), \
             patch("software.app.browser_probe._kill_process_tree") as kill_mock, \
             patch("software.app.browser_probe.time.monotonic", side_effect=[0.0, 0.0, 2.0, 2.0, 2.0]):
            result = run_browser_probe_subprocess(
                headless=True,
                browser_preference=["edge"],
                timeout_seconds=1,
    ...
    def test_run_browser_probe_subprocess_returns_cancelled_when_event_is_set(self) -> None:
        ...
        with patch("software.app.browser_probe.subprocess.Popen", return_value=fake_process), \
             patch("software.app.browser_probe._kill_process_tree") as kill_mock, \
             patch("software.app.browser_probe.time.monotonic", side_effect=[0.0, 0.1, 0.1, 0.1]):
            result = run_browser_probe_subprocess(
                headless=True,
                browser_preference=["edge"],
                timeout_seconds=1,

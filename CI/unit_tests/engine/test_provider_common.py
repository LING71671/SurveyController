...
from software.core.engine.provider_common import (
    _build_grouped_runtime_items,
...
class ProviderCommonTests(unittest.TestCase):
    def test_build_grouped_runtime_items_filters_blank_dimension(self) -> None:
        ...
        with patch(
            "software.core.engine.provider_common.build_psychometric_blueprint",
            return_value={
                "A": [_FakeBlueprintItem({"num": 1})],
                "": [_FakeBlueprintItem({"num": 2})],
            },
        ):
            grouped = _build_grouped_runtime_items(config)

        self.assertEqual(grouped, {"A": [{"num": 1}]})
    ...
    def test_build_psychometric_plan_for_run_falls_back_to_default_target_alpha_when_normalizer_errors(self) -> None:
        ...
        with patch("software.core.engine.provider_common._build_grouped_runtime_items", return_value={"A": [{"num": 1}]}), \
             patch("software.core.engine.provider_common.normalize_target_alpha", side_effect=[RuntimeError("boom"), 0.91]), \
             patch("software.core.engine.provider_common.build_dimension_psychometric_plan", return_value="plan") as build_mock:
    ...
    def test_ensure_joint_psychometric_answer_plan_builds_and_caches_value(self) -> None:
        ...
        self.assertEqual(plan, "joint-plan")
        self.assertEqual(config.joint_psychometric_answer_plan, "joint-plan")
    ...
    def test_provider_run_context_uses_explicit_plan_and_resets_persona(self) -> None:
        ...
        with patch("software.core.engine.provider_common.generate_persona", return_value={"name": "p"}), \
             patch("software.core.engine.provider_common.set_current_persona") as set_persona_mock, \
             patch("software.core.engine.provider_common._reset_answer_context") as reset_context_mock, \
             patch("software.core.engine.provider_common.reset_tendency") as reset_tendency_mock, \
             patch("software.core.engine.provider_common.reset_consistency_context") as reset_consistency_mock, \
             patch("software.core.engine.provider_common.reset_persona") as reset_persona_mock:
            with provider_run_context(config, psycho_plan="manual-plan") as resolved:
                self.assertEqual(resolved, "manual-plan")
    ...
    def test_provider_run_context_combines_joint_sample_plan_with_fallback(self) -> None:
        ...
        joint_plan = SimpleNamespace(
            build_sample_plan=lambda sample_index: SimpleNamespace(
        ...
        with patch("software.core.engine.provider_common.generate_persona", return_value={}), \
             patch("software.core.engine.provider_common.set_current_persona"), \
             patch("software.core.engine.provider_common._reset_answer_context"), \
             patch("software.core.engine.provider_common.reset_tendency"), \
             patch("software.core.engine.provider_common.reset_consistency_context"), \
             patch("software.core.engine.provider_common.build_psychometric_plan_for_run", return_value="fallback-plan"), \
             patch("software.core.engine.provider_common.ensure_joint_psychometric_answer_plan", return_value=joint_plan), \
             patch("software.core.engine.provider_common.CombinedPsychometricPlan", return_value=combined_result) as combined_mock, \
             patch("software.core.engine.provider_common.reset_persona"):
            with provider_run_context(config, state=state, thread_name="Worker-2") as resolved:
                self.assertIs(resolved, combined_result)

        combined_mock.assert_called_once()
        self.assertEqual(combined_mock.call_args.kwargs["fallback"], "fallback-plan")
        self.assertEqual(combined_mock.call_args.kwargs["primary"].choices, {1: 2})
    ...
    def test_provider_run_context_logs_global_dimension_summary_for_fallback_plan(self) -> None:
        ...
        with patch("software.core.engine.provider_common.generate_persona", return_value={}), \
             ...
             patch("software.core.engine.provider_common.reset_tendency"), \
             patch("software.core.engine.provider_common.reset_consistency_context"), \
             patch("software.core.engine.provider_common.build_psychometric_plan_for_run", return_value=fallback_plan), \
             patch("software.core.engine.provider_common.ensure_joint_psychometric_answer_plan", return_value=None), \
             patch("software.core.engine.provider_common.logging.info") as info_mock, \
             patch("software.core.engine.provider_common.reset_persona"):
            with provider_run_context(config) as resolved:
                self.assertIs(resolved, fallback_plan)

        info_call = info_mock.call_args_list[-1]
        self.assertIn("全局未分组问卷", info_call.args[-1])
